...
不要默认创建 README、CHANGELOG、安装指南、空测试目录、空脚本目录。只有当用户明确要求 GitHub 开源发布、仓库展示或贡献说明时，才添加面向人的 `README.md`；README 不能替代 `SKILL.md` 或 `references/`。
...
```bash
python3 /Users/didi/.codex/skills/.system/skill-creator/scripts/init_skill.py <skill-name> --path <output-dir> --resources references,scripts
```

只传实际需要的资源类型。初始化后删除占位内容，补充真实说明和资源。

## 5. 生成 agents/openai.yaml

推荐使用官方脚本生成 UI 元信息：

```bash
python3 /Users/didi/.codex/skills/.system/skill-creator/scripts/generate_openai_yaml.py <skill-folder> \
...
只在用户明确提供或项目需要时添加 icon、brand_color、依赖工具等字段。
